...
| 1 | 272            | 1.53          | 175.05       | 73.48        | 低回購率、消費金額較高、流失風險 | 57.54      | 刺激回購，提高回購率 | **中**     | - 提供限時折扣、體驗包<br>- 精準推播 EDM/LINE 提醒<br>- 首次回購優惠活動 |

k = 5 (最終選擇)

| **群組**    | **Recency（天）** | **Frequency** | **Monetary** | **回購機率 (%)** | **客戶特徵**         | **占比 (%)** | **預期行銷貢獻** | **行銷優先級** | **行銷策略建議**                            |
| --------- | -------------- | ------------- | ------------ | ------------ | ---------------- | ---------- | ---------- | --------- | ------------------------------------- |
| 0 | 163.42         | 1.65          | 156.98       | 73.54        | 新客剛建立關係、回購機會適中   | 34.76      | 刺激回購，提高轉換率 | **高**     | - 提供首次回購優惠<br>- 限時折扣推播<br>- 引導加入會員活動        |
| 1 | 445.73         | 1.40          | 148.98       | 75.00        | 長時間未回購、即將流失      | 24.94      | 挽回流失，提高回購率 | **中**     | - 提供高誘因折扣券<br>- 推出回流專屬優惠<br>- EDM/LINE 喚回行銷 |
| 2 | 172.52         | 5.58          | 184.81       | 91.93        | 穩定回購、高回購機率、消費偏高  | 8.51       | 穩定高利潤貢獻    | **最高**    | - 提供 VIP 專屬優惠<br>- 推薦高價商品組合<br>- 建立長期會員計畫   |
| 3 | 86.69          | 2.38          | 2806.63      | 71.81        | 超高消費但回購率偏低       | 0.42       | 高單價潛力客戶    | **中**     | - 精準推薦高價產品<br>- 高價值客戶專屬服務<br>- 關係管理加強       |
| 4 | 206.44         | 2.24          | 143.57       | 90.65        | 回購頻率高、回購機率高、消費適中 | 31.38      | 穩定收入貢獻     | **高**     | - 推薦定期購方案<br>- 會員積分兌換促銷<br>- 提升單次消費額        |
...
2. **模型預測效能中等，仍具參考價值**
   LightGBM 模型的 AUC-ROC 為 0.6216，顯示模型對「是否回購」具有一定區分能力。Precision 達 43.1%，代表模型推薦的潛在回購客戶中，有相當比例確實會回購，適合用於精準行銷與資源集中投放。Recall 為 12.65%，代表整體潛在回購者中，模型能捕捉的比例偏低，偏向保守預測策略，可避免誤判但也壓縮觸及範圍。雖然 F1-score 為 0.1956，主要受限於資料不平衡，但整體模型可協助初步篩選出潛在回購客戶。
...
5. **潛在價值客群**
   - 第 2 群與第 4 群具有 最高回購機率（皆逾 0.9），代表可優先鎖定的高潛力對象。
   - 第 3 群雖然回購率普通（0.718），但其平均消費高達 2806 元，為「高價值低頻率」族群，可透過特殊商品推薦維繫關係。
   - 第 1 群則為「低價值低活躍」族群，應謹慎投入行銷資源。
     
1. 回購率偏低具合理性
    - 本資料來自大型電商平台，涵蓋多元商品類別，包含大量低頻次或一次性購買商品（如家具、家電、節慶商品等）。
    - 整體回購率僅 3.12%，符合大型電商以長尾產品為主、客戶購買行為高度異質的特性。
   
2. 模型預測表現分析
    - AUC 為 0.6216，模型具備一定區分能力，但受到商品類別多樣性影響，整體預測效果偏弱。
    - Precision 高達 43%，適合用於高潛力客群的精準行銷策略。
    - Recall 僅 12.65%，模型偏向保守判斷，能找到的潛在回購客戶有限。
   
3. 客戶特徵洞察
   
   (1) 高消費金額客戶為影響回購的最關鍵因素
    - monetary_avg 在特徵重要性中排名第一，說明客戶的平均消費金額是影響回購意願的最主要因素。
    - 實務上，應優先針對高消費金額客戶提供 專屬優惠 和 高價值商品組合推薦，提升回購率與客單價。
   
   (2) 地區（州別）因素影響回購行為
    - customer_state_encoded 特徵重要性排名第二，顯示客戶所屬地區對回購行為有明顯影響。
    - 具體來看，AC 和 RO 州的回購率超過 9%，建議針對這些州投入更多行銷資源與促銷活動；而 CE 和 AP 州的回購率較低，可適度降低行銷資源配置或調整產品策略。
   
   (3) 付款方式影響回購行為
    - 優惠券（voucher）與信用卡（credit_card）在特徵重要性中排名較前，說明這兩類付款方式的客戶回購意願較高。
    - 根據資料分析，優惠券客戶的回購率最高，金融卡（debit_card）客戶回購率僅約 2%，建議針對優惠券與信用卡客群提供專屬回饋活動，提升忠誠度與回購率。
    - main_payment_* 特徵的重要性偏低，顯示僅看主要付款方式無法有效區分回購行為，建議可從綜合付款習慣進行分析。

4. 客群分群結果
    - 穩定高價值客戶（群組 2 & 4）回購機率超過 90%，應優先維繫，設計專屬會員計畫與回購優惠。
...
3. 區域與付款策略優化
    ...
    - 針對優惠券與信用卡付款客群，設計專屬回饋活動，提高回購率與黏著度。
